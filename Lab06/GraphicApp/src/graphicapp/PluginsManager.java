...
import Interfaces.IGraphicAppPlugin;
...
public class PluginsManager {
   private List<IGraphicAppPlugin> pluginsList = new ArrayList<IGraphicAppPlugin>();
    
    private IGraphicAppPlugin loadPlugin(File jarFile)
    {
        IGraphicAppPlugin retPlugin = null;
        try
        {
            ...
            for(String currentClassName : classesInJar)
            {
                ...
                if (IGraphicAppPlugin.class.isAssignableFrom(currentClass))
                {
                    System.out.println("Seems like class " + currentClassName + " is a plugin");
                    //we create an instance for the plugin
                    retPlugin = (IGraphicAppPlugin)currentClass.getConstructor().newInstance();
                    break;
                }                    
            }
    ...
    public void loadPlugins(String directory)
    {
        ...
        {
           IGraphicAppPlugin plugin = loadPlugin(jar);
           if (plugin != null)
           {
    ...
    public List<IGraphicAppPlugin> getPluginsList()
    {
        return pluginsList;
    } 
}
